...
class MaxPrevCountReducer extends Reducer[Text, Text, Text, IntWritable] {
  ...
  override def reduce(
    ...
    // this is a little Java style, essentially for-looping through the iterator
    // the iterator itself keeps track of our position
    val iter = values.iterator()
    var max = 0 //default to 0
    var  prevcurrPair = prev //default to just the key
    while(iter.hasNext()){
      val nextValue = iter.next().toString() //gets the next value from the iterator
      val curr = nextValue.split("\\t")(0) //grab curr field off it
